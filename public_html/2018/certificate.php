...
$result_certificate = mysql_db_query ($db,$sql_certificate);
$row_certificate = mysqli_fetch_array($result_certificate);
	...
	if($type_certificate == 1){
		$file_number_print = "<a href='http://bgl.chanthaburi.buu.ac.th/pdf.php?certificate_id=$certificate_id' target='_blank'>คลิกที่นี้เพื่อเรียกดู ใบรับรอง File Number : $file_number</a>"."<br>";
	}
	//แบบมีรูป
	if($type_certificate == 2){
		$file_number_print = "<a href='http://bgl.chanthaburi.buu.ac.th/pdf_photo.php?certificate_id=$certificate_id' target='_blank'>คลิกที่นี้เพื่อเรียกดู ใบรับรอง File Number : $file_number</a>"."<br>";
	}
	if($type_certificate == 3){
		$file_number_print = "<a href='http://bgl.chanthaburi.buu.ac.th/pdf_big.php?certificate_id=$certificate_id' target='_blank'>คลิกที่นี้เพื่อเรียกดู ใบรับรอง File Number : $file_number</a>"."<br>";
	}
?>
...
realize that there is a possibility that the gemstone may be different from this certificate. The certificate can be verified by the QR code on its or at http://bgl.chanthaburi.buu.ac.th 
</p>
