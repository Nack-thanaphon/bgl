...
$lab_status = $_REQUEST['lab_status'];
...
if ($type == "delall" and $dele != ""){
	for($i=0;$i<count($dele);$i++){
		...
		$lab_file = $row['lab_file'] ;
			if($lab_file <> ""){
				if (file_exists ("../file_lab/$lab_file")){
					unlink("../file_lab/$lab_file") ;
				}
			}
		$del_sql="delete from lab where lab_id = '$lab_id'";
		$result_del =  mysqli_query($db,$del_sql);
...
$no = 1;
$sql="select * from lab order by lab_id asc" ;
...
$result = mysqli_query($link,$sql);
	...
	$lab_status = $row['lab_status'];
		...
		if ($lab_file != ""){
			...
			$lab_file_print = "<p class='center'><a href='../file_lab/$lab_file' target='_blank' title='ดาวน์โหลดเอกสาร'><img src='images_admin/$ext.png' /></a></p>";
		} else {
			$lab_file_print = "";
		}
?>
                    <tr>
                        <td><input name="check[]" type="checkbox" id="check[]" value="<? echo"$lab_id";?>"/></td>
                        <td><?php echo"$lab_name";?></td>
                        <td><?php echo"$lab_file_print";?></td>
                        <td class="delete"><a href="lab_display.php?Page_p=<?php echo"$Page_p";?>&lab_id=<?php echo"$lab_id";?>&lab_status=<?php echo"$lab_status";?>&type=status"><?php echo"$status";?></a></td>
                        <td class="delete"><a href="lab_edit.php?Page_p=<?php echo"$Page_p";?>&lab_id=<?php echo"$lab_id";?>"><img src="images_admin/ico_edit.png" alt="" title="แก้ไข" width="16" height="16" /></a>
                        <a href="lab_display.php?Page_p=<?php echo"$Page_p";?>&lab_id=<?php echo"$lab_id";?>&file_del=<?php echo"$lab_file";?>&type=del" onclick="return confirm ('ยืนยันลบข้อมูล')"><img src="images_admin/ico_delete.png" alt="" title="ลบ" width="16" height="16" /></a></td>
                    </tr>
<?php $no++; } ?>
                </tbody>
            </table>
